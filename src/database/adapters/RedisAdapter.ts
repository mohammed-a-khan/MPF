...
export class RedisAdapter extends DatabaseAdapter {
    ...
    async connect(config: DatabaseConfig): Promise<DatabaseConnection> {
        ...
        try {
            ...
            ActionLogger.logError('Failed to connect to Redis', enhancedError);
            throw enhancedError;
        }
    }
    ...
    async disconnect(_connection: DatabaseConnection): Promise<void> {
        try {
            ...
            ActionLogger.logInfo('Disconnected from Redis');

        } catch (error) {
            const enhancedError = this.enhanceError(error as Error, 'disconnect');
            ActionLogger.logError('Failed to disconnect from Redis', enhancedError);
            throw enhancedError;
        }
    }
    ...
    async query(
        ...
        try {
            ...
            await actionLogger.logDatabase('complete', query, duration, queryResult.rowCount);
            return queryResult;

        } catch (error) {
            const enhancedError = this.enhanceError(error as Error, 'execute', { query, params });
            ActionLogger.logError('Query execution failed', enhancedError);
            throw enhancedError;
        }
    }
    ...
    async beginTransaction(_connection: DatabaseConnection, _options?: TransactionOptions): Promise<void> {
        try {
            this.validateConnection();
            ActionLogger.logInfo('Beginning Redis transaction');
            ...
            logger.debug('Redis transaction started (MULTI)');
            ActionLogger.logInfo('Redis transaction started');
    ...
    async commitTransaction(_connection: DatabaseConnection): Promise<void> {
        try {
            ActionLogger.logInfo('Committing Redis transaction');
            ...
            logger.debug('Redis transaction committed (EXEC)');
            ActionLogger.logInfo('Redis transaction committed');
    ...
    async rollbackTransaction(_connection: DatabaseConnection): Promise<void> {
        try {
            ActionLogger.logInfo('Rolling back Redis transaction');
            ...
            ActionLogger.logInfo('Redis transaction rolled back');
    ...
    async prepareStatement(query: string): Promise<PreparedStatement> {
        ...
        const preparedStatement: PreparedStatement = {
            ...
            close: async () => {
                logger.debug(`Closed prepared statement simulation: ${id}`);
            }
        };

        logger.debug(`Created simulated prepared statement for Redis: ${id}`);
    ...
    async executeBulkInsert(
        ...
        try {
            this.validateConnection();
            ActionLogger.logInfo('Bulk insert operation', { 
                key: table, 
                count: values.length 
            });
            ...
            const actionLogger = ActionLogger.getInstance();
            await actionLogger.logDatabase('bulkInsert', `BULK INSERT ${table}`, duration, insertedCount);
            return queryResult;
    ...
    async executeStoredProcedure(
        ...
        try {
            this.validateConnection();
            ActionLogger.logInfo('Executing stored procedure', { name: procedureName, params });
            ...
            const actionLogger = ActionLogger.getInstance();
            await actionLogger.logDatabase('stored_procedure', procedureName, duration, queryResult.rowCount);
            return queryResult;

        } catch (error) {
            const enhancedError = this.enhanceError(error as Error, 'executeStoredProcedure', { name: procedureName, params });
            ActionLogger.logError('Stored procedure execution failed', enhancedError);
            throw enhancedError;
        }
    }
    ...
    async executeFunction(
        ...
        try {
            this.validateConnection();
            ActionLogger.logInfo('Executing function', { name: functionName, params });

            const result = await this.executeRedisFunction(functionName, params);

            const duration = Date.now() - startTime;
            const actionLogger = ActionLogger.getInstance();
            await actionLogger.logDatabase('function', functionName, duration);
            return result;
    ...
    private async loadLuaScripts(): Promise<void> {
        ...
        logger.debug('Loaded Redis Lua scripts');
    }
    ...
    private async executeRedisCommand(command: any): Promise<any> {
        ...
        switch (cmd) {
            ...
            case 'SPOP': return args[1] ? await client.sPopCount(args[0], parseInt(args[1])) : await client.sPop(args[0]);
